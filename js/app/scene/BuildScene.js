...
function buildScene(level, scene)
{
    ...
    for (var i = 0; i < len; i++) parseObject(level[i], scene);
    {
    ...
    for (i = 0; i < len; i++) if (updateables[i].type && updateables[i].type == "AttachedImage") updateables[i].initAttachement(elementByName(updateables[i].attachedToName), scene);
}
...
function parseObject(obj, parent)
{
    ...
    switch(obj.type)
    {
        ...
        case "divWithColour":
            ...
            //displayObject.style.left="500px";
            displayObject.style.width=obj.width+"px";
            displayObject.style.height=obj.height+"px";

            displayObject.style.backgroundColor = obj.colour;
            //console.log("?????????????????"+obj.posY+"  "+displayObject.offsetLeft);
            parent.appendChild(displayObject);
            images.push(displayObject);
            break;
        case "attachedImage":
            displayObject = displayFactory.getElementByRef(obj.ref);
            displayObject.object.style.position="absolute";
            var img = new AttachedImage(obj.name, displayObject.object, obj.attachedTo, obj.offsetX, obj.offsetY, obj.placeBehind);
            updateables.push(img);
            break;
        case "triggeredAnim":   //buildTriggeredAnim(name,className,  posX, posY, cellWidth, cellHeight, numCells, target)
            var anim = new buildTriggeredAnim(obj.name, obj.ref, obj.posX, obj.posY, parent, obj.target);
            //var img = buildAttachedImage(obj.name, obj.url, obj.width, obj.height, obj.attachedTo,obj.offsetX, obj.offsetY, obj.placeBehind, parent);
        ...
        case "signAnim":
            ...
            displayObject.object.style.position="absolute";
            parent.appendChild(displayObject.object);
            var img = new SignAnim(obj.name, displayObject.object, obj.fromX, obj.fromY, obj.toX, obj.toY, obj.autostart);
            updateables.push(img);
            break;
        case "line":
            var line = new Line();
            ...
            break;
        case "bouncer":
            var bouncer=buildBouncer(obj.ref, parent, obj.posX, obj.posY, obj.cellWidth, obj.cellHeight, obj.numCells, obj.bounceYAmt, obj.bbWidth, obj.bbHeight, obj.bbOffsetX, obj.bbOffsetY, obj.speed);
            hittables.push(bouncer);
            break;
        case "blocker":
            var blocker = buildBlocker(obj.posX,obj.posY, obj.width, obj.height);
        ...
        case "pickup":
            ...
            var pickup = buildPickUp(obj.ref, parent, obj.posX, obj.posY, obj.cellWidth*MEDIA_SIZE_RATIO, obj.cellHeight*MEDIA_SIZE_RATIO, obj.numCells, obj.bbWidth, obj.bbHeight, obj.speed);
            hittables.push(pickup);
            break;
        case "switch":
            ...
            break;
        case "lethalToTouch":
            var lethal = buildLethalToTouch(obj.ref, parent, obj.posX, obj.posY, obj.cellWidth, obj.cellHeight, obj.numCells, obj.bbWidth, obj.bbHeight, obj.speed);
            hittables.push(lethal);
            break;
        case "fallingBlock":
