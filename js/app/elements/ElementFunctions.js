...
function checkForBlockers(blockers)
{
    ...
    {//Loop through blockers
        ...
        {
            if(this.contact)
            {//If in contact with line edge
                var shift = this.origPosition.x<blocker.centreX?-1:1; //Direction
                var blockerEdge = shift<0?blocker.boundingBox.x:blocker.boundingBox.x+blocker.boundingBox.width;//Pos of leading edge of blocker
                //Check for line-line intersection
                var pt = this.intersection.LineLineIntersection(this.contact.from, this.contact.to,
                    new Point(blockerEdge+shift,this.contact.from.y-100),
                    new Point(blockerEdge + shift, this.contact.to.y + 100));
                //If intersection point is returned then apply otherwise regress to the
                if(pt) this.to = pt;
                else this.to.x = this.origPosition.x;
            }
            else
            {//If not in contact with line edge
